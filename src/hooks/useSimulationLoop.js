...
export function useSimulationLoop({ isLoggedIn, s1PathRef, s2PathRef, cwPathRef, tempPathRef, conveyorRef }) {
    ...
    const runPhysicsTick = useCallback(() => {
        engine.mutate(P => {
            ...
            P.plant.y11 = a11_true * P.plant.y11 + (1 - a11_true) * (I.gain_s1 * VALVE_1_CAPACITY_VOL * u1_eff_plant);
            P.plant.y22 = a22_true * P.plant.y22 + (1 - a22_true) * (I.gain_s2 * VALVE_2_CAPACITY_VOL * u2_eff_plant);
            P.plant.y21 = a21_true * P.plant.y21 + (1 - a21_true) * (I.coupling * VALVE_1_CAPACITY_VOL * u1_delayed_eff_plant);
            ...
            P.internal_model.y11 = a11 * P.internal_model.y11 + (1 - a11) * (T.gain_s1 * VALVE_1_CAPACITY_VOL * u1_req_eff);
            P.internal_model.y22 = a22 * P.internal_model.y22 + (1 - a22) * (T.gain_s2 * VALVE_2_CAPACITY_VOL * u2_req_eff);
            P.internal_model.y21 = a21 * P.internal_model.y21 + (1 - a21) * (T.coupling * VALVE_1_CAPACITY_VOL * u1_req_delayed_eff);

            // 3. THERMAL EXCHANGER MATH
            ...
            if (P.opMode === 'AUTO' && P.silState === 'NORMAL') {
                const thermalOutput = SupervisoryAPC.solveThermal(activeTargetTemp, P.internal_model.temp_y, P.mvs.steam, P.req_mv_hist.steam, P.biases.temp, dt_sec, reqFlowLoad);
                P.mvs.steam = thermalOutput.steamMV; tempTrajectory = thermalOutput.trajectory; P.projectedTemp = tempTrajectory[tempTrajectory.length - 1];
            }
            ...
            if (bottleAtSensor) {
                ...
                if (P.opMode === 'AUTO') {
                    ...
                    if (Math.abs(P.pvsTemp - activeTargetTemp) < 3.0) {
                        ...
                        if(activeRun) { activeRun.count++; activeRun.volume += botVol; activeRun.giveawayVol += giveawayVol; activeRun.wasteVol += underfillVol; activeRun.savingsVol += savingsVol; }
                        P.cloudQueue.push({ t: now, v: botVol, m: P.pvsMass.cw, p: P.activeProduct.id }); P.cloudQueueSize = P.cloudQueue.length; P.counters.total++; 
            ...
            if (P.silState === 'RECOVERY' && bottleAtSensor) { P.recoveryStep++; if (P.recoveryStep >= RECOVERY_CYCLES) { P.silState = 'NORMAL'; P.recoveryStep = 0; } }
        });
    }, []);
    ...
    const renderVisuals = useCallback((P) => {
        ...
        if (conveyorRef.current) {
            const container = conveyorRef.current; const bottles = P.conveyor; const T_val_vol = getTolerableDeficiency(P.baseRecipeVol); const pixelsPerL = 20 / T_val_vol; const divertMode = Math.abs(P.pvsTemp - visualTargetTemp) >= 3.0;
  
            while (container.children.length < bottles.length) {
                const div = document.createElement('div');
                div.style.position = 'absolute'; div.style.top = '50%'; div.style.border = '2px solid rgba(255,255,255,0.8)'; div.style.display = 'flex'; div.style.flexDirection = 'column'; div.style.alignItems = 'center'; div.style.justifyContent = 'flex-end'; div.style.color = 'white'; div.style.fontSize = '8px'; div.style.fontWeight = 'bold'; div.style.boxShadow = '0 4px 6px -1px rgba(0, 0, 0, 0.2)'; div.style.overflow = 'hidden'; div.setAttribute('data-id', '');
                const fill = document.createElement('div'); fill.style.width = '100%'; fill.style.transition = 'height 0.1s ease'; div.appendChild(fill);
                const textSpan = document.createElement('div'); textSpan.style.position = 'absolute'; textSpan.style.top = '50%'; textSpan.style.transform = 'translateY(-50%)'; textSpan.style.textAlign = 'center'; div.appendChild(textSpan); container.appendChild(div);
            }
            ...
            bottles.forEach((bot, i) => {
                const div = container.children[i]; div.style.display = 'flex'; const fillDiv = div.children[0]; const textDiv = div.children[1];
                const botDeltaT = bot.currentTemp - 20.0; const botSG_Base = PRODUCTS[0].sg20 / (1 + PRODUCTS[0].thermalExp * botDeltaT); const botSG_Active = P.activeProduct.sg20 / (1 + P.activeProduct.thermalExp * botDeltaT); const hotSgMix = (botSG_Base * (1 - P.activeProduct.ratio)) + (botSG_Active * P.activeProduct.ratio);
                const botHotVol = bot.weightMass / hotSgMix; 
  
                if (div.getAttribute('data-id') !== bot.id) {
                    div.setAttribute('data-id', bot.id); let liquidColor = P.activeProduct.color;
                    if (divertMode) liquidColor = '#64748b'; else if (botHotVol < P.baseRecipeVol - 2*T_val_vol) liquidColor = '#ef4444'; else if (botHotVol < P.baseRecipeVol - T_val_vol) liquidColor = '#f59e0b'; 
                    div.style.backgroundColor = '#e2e8f0'; div.style.borderRadius = '4px 4px 2px 2px'; div.style.width = '20px'; div.style.height = '42px'; 
                    const fillPct = Math.min(100, Math.max(0, (botHotVol / P.baseRecipeVol) * 100)); fillDiv.style.height = `${fillPct}%`; fillDiv.style.backgroundColor = liquidColor;
                ...
                const partialTick = Math.min(1.0, (Date.now() - P.lastTick) / TICK_RATE_MS); const smoothPos = bot.position + (BELT_SPEED_PPS * partialTick); const leftPos = 8 + (smoothPos * 0.82); const dev = botHotVol - P.targetVol;
                const mergeFactor = Math.min(1, Math.max(0, (smoothPos - 70) / 20)); const finalOffset = (0 * (1 - mergeFactor)) - (dev * pixelsPerL); const clampedY = Math.max(-40, Math.min(40, finalOffset)); 
    ...
    useEffect(() => {
        ...
        const animate = (time) => {
            ...
            if (prevTimeRef.current !== undefined && P.simMode === 'RUN' && !P.eStopActive) {
                let deltaTime = time - prevTimeRef.current; if (deltaTime > 1000) deltaTime = 1000; accRef.current += deltaTime;
                while (accRef.current >= TICK_RATE_MS) { runPhysicsTick(); accRef.current -= TICK_RATE_MS; }
            }
            prevTimeRef.current = time; renderVisuals(engine.getSnapshot());
            if (time - lastRenderTime > 200) { engine.mutate(s => { s.riskAlert = s.pvsVol.cw < (s.baseRecipeVol - getTolerableDeficiency(s.baseRecipeVol)); }); engine.commit(); lastRenderTime = time; }
